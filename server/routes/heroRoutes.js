...
// ── PUBLIC: GET all (for hero slider display) ────────────────────────────────
...
// ── PROTECTED: GET all including inactive (admin) ────────────────────────────
...
router.post('/', verifyToken, (req, res) => {
  singleImageUpload(req, res, async (err) => {
    ...
    try {
      const {
        heading, subheading, slide_order, cta_text, cta_url,
        badge_text, active = true
      } = req.body;
      const image = req.file ? req.file.filename : null;
      const result = await db.query(
        `INSERT INTO hero_slides (heading, subheading, slide_order, image, cta_text, cta_url, badge_text, active)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
        [heading, subheading, slide_order || 1, image, cta_text || '', cta_url || '/', badge_text || '', active !== false && active !== 'false']
      );
      res.status(201).json({
        ...result.rows[0],
...
router.put('/:id', verifyToken, (req, res) => {
  singleImageUpload(req, res, async (err) => {
    ...
    try {
      const {
        heading, subheading, slide_order,
        cta_text, cta_url, badge_text, active
      } = req.body;

      const updates = { heading, subheading, slide_order, cta_text, cta_url, badge_text };
      if (active !== undefined) updates.active = active !== false && active !== 'false';
      if (req.file) updates.image = req.file.filename;
      if (!req.file && (req.body.remove_image === 'true' || req.body.remove_image === '1')) {
        updates.image = null;
      }
      ...
      if (result.rowCount === 0) return res.status(404).json({ error: 'Hero slide not found' });
      res.json({
        ...result.rows[0],
